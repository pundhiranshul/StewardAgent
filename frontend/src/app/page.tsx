...
export default function Home() {
  ...
  const deleteReport = async (jobIdToDelete: string) => {
    ...
    try {
      const res = await fetch(`/api/reports/${jobIdToDelete}`, {
        method: "DELETE",
        headers: { "x-access-code": accessCode }
      });
      if (res.ok) {
        setPastReports(prev => prev.filter(r => r.job_id !== jobIdToDelete));
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[95%] 2xl:max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16 flex flex-col gap-8 md:gap-12 relative z-10 print:py-0 print:px-0 print:max-w-none">
        ...
        {status === "idle" && appMode === "initial" && (
          <div className="flex flex-col items-center justify-center gap-6 mt-12 print:hidden max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150">
            <button 
              onClick={() => setAppMode("new")}
              className="bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 hover:bg-neutral-800 dark:hover:bg-neutral-200 font-bold py-4 px-10 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-xl text-lg w-full"
            >
              <FileText className="w-5 h-5" /> Generate New Report
            </button>
            <button 
              onClick={() => setAppMode("past")}
              className="bg-white/60 dark:bg-neutral-900/60 backdrop-blur-2xl border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-white hover:bg-white dark:hover:bg-neutral-800 font-bold py-4 px-10 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-lg text-lg w-full"
            >
              <Globe className="w-5 h-5" /> View Past Reports
            </button>
          </div>
        )}

        {status === "idle" && appMode === "past" && (
          <div className="flex flex-col gap-8 mt-4 print:hidden max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150">
            <div className="flex items-center gap-4 bg-white/60 dark:bg-neutral-900/60 backdrop-blur-2xl border border-white/80 dark:border-neutral-800 rounded-3xl p-6 shadow-xl w-full">
            ...
            <div className="bg-white/60 dark:bg-neutral-900/60 backdrop-blur-2xl border border-white/80 dark:border-neutral-800 rounded-3xl shadow-xl overflow-hidden w-full">
              {pastReports.length === 0 ? (
                ...
                <div className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {pastReports.map((r) => (
                    <div key={r.job_id} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 gap-4 hover:bg-white/40 dark:hover:bg-neutral-800/40 transition-colors">
                      <div>
                        <div className="text-xl font-bold text-neutral-900 dark:text-white capitalize flex items-center gap-3">
                          {r.domain || r.job_id}
                        </div>
                        <div className="text-sm text-neutral-500 dark:text-neutral-400 mt-1 flex items-center gap-2">
                          <span className="font-mono text-xs bg-neutral-200 dark:bg-neutral-800 px-2 py-0.5 rounded-md">{r.job_id}</span>
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        <button 
                          ...
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-6 rounded-lg transition-all shadow-sm text-sm"
                        >
                          View
                        </button>
                        <button 
                          onClick={() => deleteReport(r.job_id)}
                          className="text-neutral-500 hover:text-rose-600 dark:hover:text-rose-400 font-bold py-2 px-4 rounded-lg transition-all text-sm"
                        >
                          Delete
                        </button>
                      </div>
        ...
        {status === "idle" && appMode === "new" && (
          <div className="grid md:grid-cols-2 gap-12 items-center mt-4 print:hidden max-w-5xl mx-auto relative">
            <button onClick={() => setAppMode("initial")} className="absolute -top-12 left-0 flex items-center gap-2 text-sm font-bold text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors">
              ...
              <div className="flex items-center gap-3">
                ...
                {status === "completed" && (
                  <>
                    ...
                    <button onClick={() => { resetAnalysis(); setAppMode("initial"); }} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-300 bg-white dark:bg-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-700 border border-neutral-300 dark:border-neutral-700 rounded-lg transition-colors shadow-sm">
